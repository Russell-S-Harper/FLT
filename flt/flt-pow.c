...
/* Convenience function to handle pow special cases. Note +Inf^u, t^0, and 1^u
 * are already handled. */
static FLT flt_tmp_pow_alt(flt_tmp *pt, flt_tmp *pu) {
   flt_tmp v;
   switch (pt->c) {
      ...
      */
      case E_INFINITE:
         ...
         if (flt_tmp_compare(pt, &v, E_EQUAL_TO))
            ...
            pt->s = v.s = 0;
            return flt_tmp_compare(pt, &v, pu->s ? E_LESS_THAN : E_GREATER_THAN)
                       ? FLT_POS_INF
                       : FLT_POS_0;
         }
      }
...
/* To handle t^u where u is an integer. Note +Inf^u, t^0, and 1^u are already
 * handled. */
static void flt_tmp_powN(flt_tmp *pt, flt_tmp *pu) {
   flt_tmp v, w;
   int sign;
   ...
   if (pt->c == E_NORMAL) {
      ...
      case E_ZERO:
         if (pu->s)
            /*
                    pow(+0, u), where u is a negative odd integer, returns
               +Inf pow(+0, u), where u is a positive odd integer, returns
               +0 pow(-0, u), where u is a negative odd integer, returns
               -Inf pow(-0, u), where u is a positive odd integer, returns
               -0
            */
            pt->c = E_INFINITE;
         if (!u_is_odd)
            /*
                    pow(+/-0, u), where u is a negative even integer,
               returns +Inf pow(+/-0, u), where u is a positive even
               integer, returns +0
            */
            pt->s = 0;
         break;
      case E_INFINITE:
         if (pu->s)
            /*
                    pow(-Inf, u), where u is a negative odd integer, returns
               -0 pow(-Inf, u), where u is a positive odd integer, returns
               -Inf
            */
            pt->c = E_ZERO;
         if (!u_is_odd)
            /*
                    pow(-Inf, u), where u is a negative even integer,
               returns +0 pow(-Inf, u), where u is a positive even integer,
               returns +Inf
            */
            pt->s = 0;
         break;
