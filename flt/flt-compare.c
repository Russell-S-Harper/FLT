...
bool flt_tmp_compare(const flt_tmp *pt, const flt_tmp *pu, E_COMPARE compare) {
  flt_tmp v, w;
  /* IEEE 754 specifically dictates that +Infinity == +Infinity and -Infinity ==
   * -Infinity */
  if (pt->c == E_INFINITE && pu->c == E_INFINITE && pt->s == pu->s)
    return (compare == E_EQUAL_TO || compare == E_LESS_THAN_OR_EQUAL_TO ||
            compare == E_GREATER_THAN_OR_EQUAL_TO);
  ...
  default:
    if (v.s)
      return (compare == E_LESS_THAN || compare == E_LESS_THAN_OR_EQUAL_TO ||
              compare == E_NOT_EQUAL_TO);
    else
      return (compare == E_GREATER_THAN ||
              compare == E_GREATER_THAN_OR_EQUAL_TO ||
              compare == E_NOT_EQUAL_TO);
  }
}
