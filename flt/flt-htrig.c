...
FLT flt_tanh(const FLT f) {
  ...
  flt_tmp t, u, v;
  flt_to_tmp(&f, &t);
  /* tanh(t) = (e^t - e^-t)/(e^t + e^-t) = (2^kt - 2^-kt)/(2^kt + 2^-kt), where
   * k = 1/log(2) */
  /* At flt_tmp precision, |tanh(t)| -> 1 for |t| >= 10 */
  flt_tmp_initialize(&u, E_NORMAL, t.s, 0x50000000, 3); /* 10.0 */
  if (t.c == E_NORMAL &&
      flt_tmp_compare(&t, &u, t.s ? E_GREATER_THAN : E_LESS_THAN)) {
    flt_tmp_initialize(&u, E_NORMAL, 0, TMP_1_LOG2, 0); /* k = 1/log(2) */
    flt_tmp_multiply(&t, &u);                           /* kt */
    flt_tmp_copy(&u, &t);
    flt_tmp_exp2(&t);   /* 2^kt = e^t */
...
FLT flt_asinh(const FLT f) {
  ...
  if (t.c == E_NORMAL) {
    /* The positive side is much more accurate, so use asinh(-t) = -asinh(t) for
     * t < 0 */
...
/* The usable range of this function is between 0 <= |f| <= 0.999; above 0.999
 * the errors become increasingly significant. */
FLT flt_atanh(const FLT f) {
  ...
  if (t.c == E_NORMAL && t.e < 0) {
    ...
    flt_tmp_initialize(&u, E_NORMAL, 0, TMP_2LOG2,
                       -2);   /* 2log(2)/4 = log(2)/2 */
