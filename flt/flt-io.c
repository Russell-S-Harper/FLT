...
/* Maximum "E" format can be 17 characters including the null terminator: +#.#########E+##_ */
#define FMT_E_MAX_LEN	17
#define FMT_E_BFR_LEN	(15 * FMT_E_MAX_LEN)	/* Can provide up to 15 "E" calls in one printf statement (15 * 17 = 255) */
...
static int flt_tmp_normalize_to_base_10(flt_tmp *pt, int precision, const E_ROUNDING rounding);
...
static flt_tmp
	k1, k2, k3, k5,
	kn1, kn2, kn3, kn5,
	k10, k100, k1e3, k1e5, k1e8,
	k1en1, k1en2, k1en3, k1en5, k1en8,
	k5en1;
...
static const char *flt_tmp_e_format(flt_tmp *pt, const char *format) {
	...
	/* Using a larger buffer to allow multiple calls in one printf statement, i.e. up to 15. */
...
static const char *flt_tmp_f_format(flt_tmp *pt, const char *format) {
	...
	/* Using a larger buffer to allow multiple calls in one printf statement, i.e. up to 5. */
...
static const char *flt_tmp_g_format(flt_tmp *pt, const char *format) {
	...
	return flt_tmp_compare(&u, &k1e5, E_GREATER_THAN_OR_EQUAL_TO)
		|| flt_tmp_compare(&u, &k1en5, E_LESS_THAN)? flt_tmp_e_format(pt, format): flt_tmp_f_format(pt, format);
}
...
static int flt_tmp_normalize_to_base_10(flt_tmp *pt, int precision, const E_ROUNDING rounding) {
	...
	/* Get rounding, i.e. start with half and divide down by the precision. The reason
		...
		representations in base 2, so the input rarely has terminating 0s. Not worth
		the trouble for the few cases where it might apply!
	*/
...
static int printf_precision(const char *format) {
	...
	return decimal_marker && isdigit(decimal_marker[1])? decimal_marker[1] - '0': 6;
}
...
char *flt_get_scanf_buffer(int offset) {
	/* Using a larger buffer to allow multiple calls in one scanf statement, i.e. up to 5. */
	/* Assumes the worst case of the longest possible legal input! */
	static char _strings[FMT_F_BFR_LEN];
	int index = (FMT_F_MAX_LEN * abs(offset)) % FMT_F_BFR_LEN;
	/* A negative offset means get the buffer for writing into, so initialize to NaN */
	if (offset < 0)
