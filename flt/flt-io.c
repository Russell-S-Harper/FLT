...
#define FMT_F_BFR_LEN                                                          \
    (5 * FMT_F_MAX_LEN) /* Can provide up to 5 "F" calls in one printf         \
                           statement (5 * 51 = 255) */
        ...
        default:
            if (isdigit(string[i])) {
                ...
                case E_INTEGER:
                    ...
                    if (string[i] > '0') {
                        flt_tmp_initialize(&u, E_NORMAL, 0, string[i] - '0',
                                           TMP_1_BITS);
                        flt_tmp_normalize(&u);
                        flt_tmp_add(&t, &u);
                    }
...
static const char *flt_tmp_e_format(flt_tmp *pt, const char *format) {
    ...
    /* Using a larger buffer to allow multiple calls in one printf statement,
     * i.e. up to 15. */
...
static int printf_precision(const char *format) {
    ...
    return decimal_marker && isdigit(decimal_marker[1])
               ? decimal_marker[1] - '0'
               : 6;
}
...
static char *printf_post_process(char *string, const char *format,
    ...
    if (!precision && (decimal_marker = strchr(string, '.')))
        memmove(decimal_marker, decimal_marker + 1,
                strlen(decimal_marker + 1) + 1);
...
char *flt_get_scanf_buffer(int offset) {
    /* Using a larger buffer to allow multiple calls in one scanf statement,
     * i.e. up to 5. */
