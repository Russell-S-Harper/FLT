...
void flt_tmp_sqrt_ext(flt_tmp *pt) {
   if (pt->c == E_NORMAL) {
      if (!pt->s)
         ...
         flt_tmp_initialize(pt, E_ZERO, 0, 0,
                            0); /* Forcing t < 0 to 0 due to rounding errors */
   }
...
void flt_tmp_sqrt(flt_tmp *pt) {
   ...
   if (pt->m > TMP_1) {
      ...
      flt_tmp_copy(&u, pt);
      flt_tmp_initialize(&v, E_NORMAL, 0, 0x6975E171,
                         -2); /* 0.411954965695194 */
      flt_tmp_multiply(pt, &v);
      flt_tmp_initialize(&v, E_NORMAL, 0, 0x4CE47AAF,
                         -1); /* 0.600722632796001 */
      flt_tmp_add(pt, &v);    /* x */
      for (i = 0; i < 3; ++i) {
         flt_tmp_copy(&v, pt);     /* x */
   ...
   if (exponent & 1) {
      ...
      flt_tmp_initialize(&u, E_NORMAL, 0, 0x5A82799A, 0);
