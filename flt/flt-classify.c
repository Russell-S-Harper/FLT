...
bool flt_isinf(const FLT f) {
	return flt_classify(f) == E_INFINITE;
}

bool flt_isnan(const FLT f) {
	return flt_classify(f) == E_NAN;
}

bool flt_isnormal(const FLT f) {
	return flt_classify(f) == E_NORMAL;
}

bool flt_issubnormal(const FLT f) {
	return flt_classify(f) == E_SUBNORMAL;
}

bool flt_iszero(const FLT f) {
	return flt_classify(f) == E_ZERO;
}
